...
public class Overworld : Scene
{
	...
	public class Entry
	{
		...
		public void Redraw(Batcher batch, float shine)
		{
			...
			batch.SetSampler(new(TextureFilter.Nearest, TextureWrap.ClampToEdge, TextureWrap.ClampToEdge));
	...
	public override void Render(Target target)
	{
		...
		for (int i = 0; i < entries.Count; i ++)
		{
            ...
            if (material.Shader?.Has("u_texture_sampler") ?? false)
			    material.Set("u_texture_sampler", new TextureSampler(TextureFilter.Nearest, TextureWrap.ClampToEdge, TextureWrap.ClampToEdge));
		...
		// overlay
		{
			batch.SetSampler(new TextureSampler(TextureFilter.Nearest, TextureWrap.ClampToEdge, TextureWrap.ClampToEdge));
			...
			batch.PushSampler(new TextureSampler(TextureFilter.Nearest, TextureWrap.Repeat, TextureWrap.Repeat));
